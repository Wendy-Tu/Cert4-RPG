...
public class PlayerStats : MonoBehaviour {
    ...
    public Text youWinText, xpText, goldTxt;
    ...
    void Update () {
        ...
        #region Rewarding Completed Quests
        //get a reference to our player quest object
        PlayerQuest pq = GetComponent<PlayerQuest>();

        //cycle
        for (int i = 0; i < pq.quests.Count; i++)
        {
            if (pq.quests[i].state == QuestState.Completed)
            {
                ...
                goldTxt.text = pq.quests[i].goldReward + "";
                pq.quests[i].state = QuestState.Claimed;
